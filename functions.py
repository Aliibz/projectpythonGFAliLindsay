    ...
    for fichier in os.listdir(dossier_corpus):
        if fichier.endswith('.txt'):
            ...
            with open(chemin_fichier, 'r', encoding='utf-8') as f:
                ...
                mots_trouver.update(mot for mot in mots_question if mot in mots_corpus)

    return mots_trouver



def TFIDFQuestion(question, scores_idf):
    ...
    for mot in mots_question:
        ...
        # Vérifier si le mot est présent dans les clés de scores_idf
        if mot in scores_idf:
...
# Obtenez le répertoire actuel du script Python
repertoire_script = os.path.dirname(os.path.realpath(__file__))

# Demander à l'utilisateur de spécifier le chemin du dossier corpus
dossier_corpus = os.path.join(repertoire_script, "cleaned")

# Utiliser le dossier_corpus dans votre programme
...
def document_plus_pertinent(question_vector, tf_idf_matrix, document_names):
    """Trouve le document le plus pertinent à partir de la similarité"""
    scores_similarite = []
...
def generer_reponse(nom_document_pertinent, mot_max_tfidf):
    """Génère une réponse à partir du document pertinent"""
